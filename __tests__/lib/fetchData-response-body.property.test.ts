...
describe("Feature: fetchdata-error-handling, Property 4: Error response body preservation", () => {
  ...
  it("Property 4a: FetchError.responseBody equals original body text for any error response", async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.integer({ min: 400, max: 599 }),
        fc.string(),
        async (status, bodyText) => {
  ...
  it("Property 4b: responseBody is null when response.text() times out (exceeds 5 seconds)", async () => {
    vi.useFakeTimers();
