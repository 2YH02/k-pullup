    ...
    <div className="space-y-1.5">
      {data.slice(0, visibleCount).map((item, index) => {
        ...
        return (
          ...
          >
            ...
            {rank === 1 && (
              <div className="mr-2 shrink-0 rounded-md bg-white/75 p-0.5 ring-1 ring-yellow/40 dark:bg-black-light">
                <Crown
                  size={14}
      ...
      <div className="flex justify-center gap-3 pt-1">
        {visibleCount > 10 && (
          <button
            onClick={resetList}
            className="rounded-full border border-border px-3 py-1 text-sm text-text-on-surface-muted transition-colors web:hover:bg-primary-subtle dark:border-grey-dark dark:text-grey-light dark:web:hover:bg-black/35"
          >
            접기
          </button>
        )}
